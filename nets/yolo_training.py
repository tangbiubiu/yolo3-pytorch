...
class YOLOLoss(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_size, cuda):
        ...
        self.ignore_threshold = 0.5 # 与gt的iou超过ignore_threshold则被忽略，低于ignore_threshold则判定为负例。
    ...
    def forward(self, input, targets=None):
        ...
        #  losses.
        loss_x = torch.sum(BCELoss(x, tx) / bs * box_loss_scale * mask) # 先验框中心的LOSS使用的交叉熵
        loss_y = torch.sum(BCELoss(y, ty) / bs * box_loss_scale * mask)
        loss_w = torch.sum(MSELoss(w, tw) / bs * 0.5 * box_loss_scale * mask) # 先验框宽高的LOSS使用均方差
        loss_h = torch.sum(MSELoss(h, th) / bs * 0.5 * box_loss_scale * mask)

        loss_conf = torch.sum(BCELoss(conf, mask) * mask / bs) + \
                    torch.sum(BCELoss(conf, mask) * noobj_mask / bs) # 先验框置信度LOSS使用交叉熵
                    
        loss_cls = torch.sum(BCELoss(pred_cls[mask == 1], tcls[mask == 1])/bs) # 类别置信度LOSS使用交叉熵

        loss = loss_x * self.lambda_xy + loss_y * self.lambda_xy + \
                loss_w * self.lambda_wh + loss_h * self.lambda_wh + \
                loss_conf * self.lambda_conf + loss_cls * self.lambda_cls # 将LOSS进行相加作为这个特征层最终的LOSS
    ...
    def generate(self, train=True):
        ...
        while True:
            ...
            for annotation_line in lines:  
                img,y=self.get_random_data(annotation_line,self.image_size[0:2]) # get_random_data()用于数据增强

                if len(y)!=0:
                    boxes = np.array(y[:,:4],dtype=np.float32)
                    boxes[:,0] = boxes[:,0]/self.image_size[1] # 下面四行是对数据进行归一化操作
                ...
                inputs.append(np.transpose(img/255.0,(2,0,1))) # 对图片进行归一化，进行通道顺序的调整
                targets.append(np.array(y,dtype = np.float32))
                if len(targets) == self.batch_size: # 如果处理的图片数已经等于batchsize的话
