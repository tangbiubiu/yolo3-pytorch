...
def fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            with torch.no_grad():
                if cuda:
                    images = Variable(torch.from_numpy(images).type(torch.FloatTensor)).cuda() # 转换为Variable的格式
...
if __name__ == "__main__":
    ...
    for i in range(3): # 因为yolov3的网络会输出三个有效特征层，所以LOSS函数要循环三次
        yolo_losses.append(YOLOLoss(np.reshape(Config["yolo"]["anchors"],[-1,2]),
                                    Config["yolo"]["classes"], (Config["img_w"], Config["img_h"]), Cuda)) # YOLOLoss()是计算loss所用的函数
    ...
    if True:
        ...
        lr_scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=1,gamma=0.95) # 学习率梯度下降的方式
        ...
        if Use_Data_Loader:
            ...
            gen = Generator(Batch_size, lines[:num_train],
                             (Config["img_h"], Config["img_w"])).generate() # 生成训练数据
            gen_val = Generator(Batch_size, lines[num_train:],
                             (Config["img_h"], Config["img_w"])).generate() # 生成验证数据
        ...
        for epoch in range(Init_Epoch,Freeze_Epoch): # 开始训练
            fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,gen_val,Freeze_Epoch,Cuda) # fit_ont_epoch()用于训练的函数
            lr_scheduler.step()
    ...
    if True: # 解冻之后的训练
        ...
        for epoch in range(Freeze_Epoch,Unfreeze_Epoch):
            fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,gen_val,Unfreeze_Epoch,Cuda) # fit_ont_epoch()用于训练的函数
            lr_scheduler.step()
